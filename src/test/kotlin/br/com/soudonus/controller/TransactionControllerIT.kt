...
class TransactionControllerIT : BaseIntegrationTest() {
    ...
    companion object {
        ...
        private val NEGATIVE_DEPOSIT = ResourceDataMapper.getFrom("transactions/negative_deposit.json", DepositCreateDTO::class.java) as DepositCreateDTO
    }
    ...
    @Test
    fun `given an negative deposit should failed`() {
        this.webTestClient.post()
                .uri(API_PATH_DEPOSIT)
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
                .bodyValue(NEGATIVE_DEPOSIT)
                .exchange()
                .expectStatus().isBadRequest
                .expectBody()
                .jsonPath("$.status").isEqualTo(HttpStatus.BAD_REQUEST.value())
                .jsonPath("$.message").isEqualTo("amount must be greater than 0")
    }
